...
const Header = ({
  ...
  const navDrawerClsNameExt = isNavDrawerOpen
    ? "right-0 shadow-[-15px_0_64px_0_rgba(163,255,18,0.15)]"
    : "right-[-300px]";
  const hamburgerSpan1ClsNameExt = isNavDrawerOpen ? "opacity-0 " : "";
  const hamburgerSpan2ClsNameExt = isNavDrawerOpen
    ? "-rotate-45 translate-x-[5px] translate-y-[10px]"
    : "";
  const hamburgerSpan3ClsNameExt = isNavDrawerOpen
    ? "rotate-45 translate-x-[4px] translate-y-[-12px]"
    : "";
  return (
    <>
      <div className="h-[100px] border-dark-grey border-0 border-b-[1px] w-full m-0 py-[30px] flex justify-between items-center">
        ...
        <div
          className="w-[20px] h-[20px] cursor-pointer z-30 xmd:hidden"
          onClick={() => toggleNavDrawer(!isNavDrawerOpen)}
        >
          <span
            className={`relative block w-[24px] h-[2px] mb-[7px] bg-white z-30 transition  origin-[0%_0%] ${hamburgerSpan1ClsNameExt}`}
          ></span>
          <span
            className={`relative block w-[24px] h-[2px] mb-[7px] bg-white z-30 transition  origin-[0%_100%] ${hamburgerSpan2ClsNameExt}`}
          ></span>
          <span
            className={`relative block w-[24px] h-[2px] mb-[7px] bg-white z-30 transition  origin-[4px_0px] ${hamburgerSpan3ClsNameExt}`}
          ></span>
        </div>
      ...
      <div
        className={`fixed top-0 w-[300px] ${navDrawerClsNameExt} h-[100vh] bg-black z-20 transition transition-all duration-300 ease-in-out flex items-center`}
      >
